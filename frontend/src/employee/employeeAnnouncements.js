// src/employee/EmployeeAnnouncements.jsx
...
const EmployeeAnnouncements = ({ user }) => {
  ...
  const loadAnnouncements = useCallback(async () => {
    setLoading(true);
    try {
      console.log('📡 Employee fetching announcements...');
      const response = await employeeApi.getActiveAnnouncements();
  ...
  const markAsRead = async (id) => {
    try {
      ...
      const response = await employeeApi.markAnnouncementRead(id);
  ...
  return (
    <div className="employee-announcements-container">
      ...
      <div className="announcements-filters">
        <button className={`filter-btn ${filter === 'all' ? 'active' : ''}`} onClick={() => setFilter('all')}>All ({announcements.length})</button>
        <button className={`filter-btn ${filter === 'unread' ? 'active' : ''}`} onClick={() => setFilter('unread')}>Unread ({unreadCount})</button>
        <button className={`filter-btn ${filter === 'read' ? 'active' : ''}`} onClick={() => setFilter('read')}>Read ({announcements.length - unreadCount})</button>
      </div>
